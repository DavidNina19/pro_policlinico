...
class DetalleMedicoSerializer(serializers.ModelSerializer):
    ...
    def get_nomMed(self, obj):
        try:
            response = requests.get(f'http://getmedico:5000/listar/medico/?nomMed={obj.codMed}/')
    ...
    def get_fecHora(self, obj):
        try:
            response = requests.get(f'http://gethorario:5000/listar/horario/?numHora={obj.numHor}/')  # Asegúrate del nombre del contenedor
            if response.status_code == 200:
                data = response.json()
                if isinstance(data, list) and data:
                    return data[0]
